...
class Pong {
    ...
    handleBallMove() {
        ...
        } else if (this.state.ball.position[1] === 0 || this.state.ball.position[1] === 1) {
            this.setBallAngle(false);
        }
    }

    ballHitMatka() {
        if (
            this.state.ball.position[0] == 0 &&
            (this.state.ball.position[1] > this.state.matkot[0].yPosition &&
                this.state.ball.position[1] < this.state.matkot[0].yPosition + MATKA_SIZE)
        ) {
            return true;
        }
        if (
            this.state.ball.position[0] == 1 &&
            (this.state.ball.position[1] > this.state.matkot[1].yPosition &&
                this.state.ball.position[1] < this.state.matkot[1].yPosition + MATKA_SIZE)
        ) {
            return true;
        }
    }
